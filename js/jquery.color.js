...
(function($) {
    ...
    var models = {
        ...
        HSL: {
            ...
            // HSLColor toString
            toString: function () {
                return 'hsl(' + this.h.toFixed() + ', ' + this.s.toFixed() + '%, ' + this.l.toFixed() + '%)';
            }
        }
    }; // models
    ...
    QColor.prototype.mean = function() {
        var that = this,
            color = new ($[ this.model.toUpperCase() + 'Color' ])(0,0,0);
        var mean = color.channelNames.map(function(channelName) {
            var range = that[ channelName ][0];
            color[ channelName ] = (range.first() + range.last()) / 2;
        });
        return color;
    };

    QColor.prototype.first = function() {
        var that = this,
            color = new ($[ this.model.toUpperCase() + 'Color' ])(0,0,0);
        var mean = color.channelNames.map(function(channelName) {
            var range = that[ channelName ][0];
            color[ channelName ] = range.first();
        });
        return color;
    };

    QColor.prototype.last = function() {
        var that = this,
            color = new ($[ this.model.toUpperCase() + 'Color' ])(0,0,0);
        var mean = color.channelNames.map(function(channelName) {
            var range = that[ channelName ][0];
            color[ channelName ] = range.last();
        });
        return color;
    };
    ...
    /*
     * Public properties added to $ object
     */

    $.Range = Range;
    // export Color subclasses based on models
    Object.keys(models).forEach(function(model) {
        $[ model + 'Color' ] = SUBCLASS(Color, models[ model ]);
    });

    // QColor definitions
    $.color_definitions = colors;

    // method for matching specific colors agains definitions
    $.perception = function(color) {
        var result = [];
        colors.forEach(function(qcolor) {
            if (qcolor.match(color)) {
                result.push(qcolor.label);
            }
        });
        if (result.length === 0 || result.length > 1) {
            throw new Error(result.length + ' perceptions [' + result.join(', ') + '] from ' + color.toString());
        }
        return result[0];
    };
